...
export default function ProjectCard({ id, name, image, isDarkMode }) {
  return (
    <Link to={`/collection/${id}`} className="group block">
      <div className={`relative overflow-hidden rounded-2xl border transition-all duration-500 hover:border-[#D17B57]/50 hover:shadow-[0_0_30px_rgba(209,123,87,0.15)] hover:-translate-y-2 ${isDarkMode ? 'bg-[#1A1A1A] border-white/5' : 'bg-[#EAE0D5] border-[#4A3224]/10'}`}>
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
          <h3 className={`text-xl font-black font-serif tracking-wide mb-1 drop-shadow-md transition-colors duration-500 ${isDarkMode ? 'text-[#FDF8F5]' : 'text-[#4A3224]'}`}>
            {name}
          </h3>
