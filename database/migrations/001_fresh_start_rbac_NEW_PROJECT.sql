-- ============================================================================
-- BOLO PANDAYAN DATABASE SETUP FOR NEW SUPABASE PROJECT
-- ============================================================================
-- Complete schema creation with normalized structure and Row-Level Security (RLS)
-- 
-- USE THIS VERSION IF: Starting with a brand new, empty Supabase project
-- DO NOT use if: Migrating from existing schema (use the other migration file instead)
--
-- INSTRUCTIONS:
...
-- 4. Click "Run"
-- 5. Verify using queries at bottom of file
--
-- WHAT THIS DOES:
-- - Creates normalized tables with proper FKs + constraints
...
-- Role hierarchy: developer (master) → lgu_admin (regional) → artisan (individual)
...
comment on table public.tbl_user_profiles is 'Master user directory. Role hierarchy: developer (master) > lgu_admin (regional) > artisan. Region field required for LGU admins.';
...
-- ============================================================================
-- PART 2: CREATE INDEXES (Performance)
-- ============================================================================
...
comment on function public.is_developer() is 'Returns true if authenticated user has developer role (can approve LGU admins).';
...
-- ============================================================================
-- PART 5: SET UP RLS POLICIES (RBAC)
-- ============================================================================
...
-- Policy: Developers can update any profile (approve LGU admins, manage artisans)
...
-- ============================================================================
-- PART 6: FIELD-LEVEL PROTECTION (Prevent users from changing their own role/status)
-- ============================================================================
...
-- ============================================================================
-- PART 7: TIGHTEN TABLE GRANTS (Optional)
-- ============================================================================
...
-- Policy: Authenticated users can update their own files in bolos
create policy "Authenticated update own bolos"
on storage.objects
for update
to authenticated
using (
...
-- Expected: all relrowsecurity should be 'true'
...
-- Breakdown:
--   tbl_user_profiles: 7 (2x SELECT + 2x INSERT (anon+authenticated) + 1x UPDATE self + 1x UPDATE developer + 1x UPDATE lgu_admin->artisan)
--   tbl_workshops: 4 (1x SELECT + 1x INSERT + 1x UPDATE + 1x DELETE)
--   tbl_products: 4 (1x SELECT + 1x INSERT + 1x UPDATE + 1x DELETE with LGU override)
--   tbl_damage_reports: 2 (1x SELECT + 1x ALL for write)
--   tbl_workshop_risk_assessments: 2 (1x SELECT + 1x ALL for developer/LGU write)
...
-- Expected: proname = 'is_developer', prosecdef = 'f'
...
-- Expected: proname = 'is_lgu_admin_approved', prosecdef = 'f'
...
-- Expected: proname = 'protect_profile_fields', prosecdef = 't' (security definer)


-- Count our custom trigger (protect_profile_updates)
select count(*) as custom_trigger_count from pg_trigger 
where tgname = 'protect_profile_updates';

-- Expected: 1 (Supabase may have internal triggers, so this specifically checks for ours)

-- To see ALL triggers on these tables (includes Supabase internals):
select tgname, tgrelid from pg_trigger 
where tgrelid in (
